...
# SOLUCIÓN: Agregamos 'strict_slashes=False' para que acepte la URL con o sin barra al final
@app.route('/registrar-voto', methods=['POST'], strict_slashes=False)
def registrar_voto():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Si Turso da error (ej: el hash está duplicado)
            return jsonify({"status": "error", "message": "El voto ya fue registrado anteriormente"}), 400
            
    except Exception as e:
        return jsonify({"status": "error", "message": f"Error interno: {str(e)}"}), 500

# MANEJO DE ERRORES EN JSON: Si algo falla en Flask, responderá JSON en vez de HTML
